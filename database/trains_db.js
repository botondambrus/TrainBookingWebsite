...
async function insertTrain(departureTime, arrivalTime, departure, arrival, day, type, price) {
  ...
  try {
    const result = await executeQuery(insertTrainQuery, [
      departureTime,
      arrivalTime,
      departure,
      arrival,
...
async function searchTrains(departure, arrival, minimumPrice, maximumPrice) {
  let searchTrainsQuery = `
      SELECT id, departure, arrival, departureTime, arrivalTime, day, type, price, TIMEDIFF(arrivalTime, departureTime) AS 'totalTime'
      FROM Trains
      WHERE 1 = 1
    `;
  ...
  if (departure) {
    searchTrainsQuery += ' AND departure = ?';
    values.push(departure);
  }

  if (arrival) {
    searchTrainsQuery += ' AND arrival = ?';
    values.push(arrival);
  }

  if (minimumPrice) {
    searchTrainsQuery += ' AND price >= ?';
...
async function getTrainsWith2Transfers(departure, arrival, minimumPrice, maximumPrice) {
  ...
  SELECT CONCAT(t1.id, '->', t2.id, '->', t3.id) AS transfers, t1.departure, t3.arrival, t1.departureTime, t3.arrivalTime, t1.day, t1.type, t1.price + t2.price + t3.price AS price, TIMEDIFF(t3.arrivalTime, t1.departureTime) AS 'totalTime'
  FROM Trains t1
  ...
  if (departure) {
    getTrainsWith2TransfersQuery += ' AND t1.departure = ?';
    values.push(departure);
  }

  if (arrival) {
    getTrainsWith2TransfersQuery += ' AND t3.arrival = ?';
    values.push(arrival);
  }

  if (minimumPrice) {
    getTrainsWith2TransfersQuery += ' AND (t1.price + t2.price + t3.price) >= ?';
    values.push(minimumPrice);
  }
...
async function getTrainsWith1Transfer(departure, arrival, minimumPrice, maximumPrice) {
  ...
  SELECT CONCAT(t1.id, '->', t2.id) AS transfer, t1.departure, t2.arrival, t1.departureTime, t2.arrivalTime, t1.day, t1.type, t1.price + t2.price AS price, TIMEDIFF(t2.arrivalTime, t1.departureTime) AS totalTime
  FROM Trains t1
  INNER JOIN Trains t2 ON t1.arrival = t2.departure
  ...
  if (departure) {
    getTrainsWith1TransferQuery += ' AND t1.departure = ?';
    values.push(departure);
  }

  if (arrival) {
    getTrainsWith1TransferQuery += ' AND t2.arrival = ?';
    values.push(arrival);
  }

  if (minimumPrice) {
    getTrainsWith1TransferQuery += ' AND (t1.price + t2.price) >= ?';
    values.push(minimumPrice);
  }
